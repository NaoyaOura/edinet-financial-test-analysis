...
public class DocumentListFetcher {
     ...
     * 土日はスキップする。
     * 業種コードは parse-xbrl フェーズで XBRL から取得するため、ここでは登録しない。
     *
     ...
     * 業種コードは parse-xbrl フェーズで更新するため、企業マスタは名前のみ登録する。
     */
    private int fetchAndSaveForDate(String date, int fiscalYear) throws Exception {
        ...
        try (Connection conn = dbManager.getConnection()) {
            ...
            for (JsonNode doc : results) {
                ...
                // 企業名のみ登録（業種コードは parse-xbrl フェーズで更新）
                companyDao.upsert(edinetCode, filerName);
